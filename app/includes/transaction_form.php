<form method="post" action="<?= $postAction ?>">
    ...
    <div class="mb-3">
        ...
        <input type="date" class="form-control <?= field_invalid_class($validationErrors, 'transaction_date') ?>"
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control <?= field_invalid_class($validationErrors, 'transaction_title') ?>"
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control <?= field_invalid_class($validationErrors, 'transaction_amount') ?>"
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" id="transaction-note" name="transaction_note"
            rows="3"><?= $old['transaction_note'] ?? '' ?></textarea>
    </div>
    <div class="mb-3">
        <div class="form-group">
            ...
            <select class="form-select" id="category" name="transaction_category_id">
                <?php foreach ($categories as $category): ?>
                    <option value="<?= $category["category_id"] ?>" <?= (isset($old["transaction_category_id"]) && ($old["transaction_category_id"] == $category["category_id"])) ? 'selected' : '' ?>>
